...
function xx2<Payload, EventType extends string = string>(eventType:EventType):EventType {
    return eventType;
}
type txx<Payload, EventType extends string>={
    eventType:EventType;
    emit(payload:Payload):void;
}
function xx<EventType extends string>(eventType:EventType): (<Payload>() => txx<Payload,EventType>) {
    return () => ({eventType:eventType, emit:(p)=>{}});
}

const abc=xx("xxx")<number>();
*/